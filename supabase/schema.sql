...
-- Drop old policies if they exist, to prevent errors on re-run
DROP POLICY IF EXISTS "Public profiles are viewable by everyone." ON public.profiles;
DROP POLICY IF EXISTS "Users can insert their own profile." ON public.profiles;
DROP POLICY IF EXISTS "Users can update own profile." ON public.profiles;
DROP POLICY IF EXISTS "Public read access for news_posts" ON public.news_posts;
DROP POLICY IF EXISTS "Public read access for jockeys" ON public.jockeys;
DROP POLICY IF EXISTS "Public read access for trainers" ON public.trainers;
DROP POLICY IF EXISTS "Public read access for horses" ON public.horses;
...
-- -------------------------------------------------
-- 4. Sample Data (runs only if tables are empty)
-- -------------------------------------------------
DO $$
BEGIN
  IF NOT EXISTS (SELECT 1 FROM public.news_posts LIMIT 1) THEN
...
DO $$
BEGIN
  IF NOT EXISTS (SELECT 1 FROM public.jockeys LIMIT 1) THEN
...
DO $$
BEGIN
  IF NOT EXISTS (SELECT 1 FROM public.trainers LIMIT 1) THEN
...
DO $$
BEGIN
  IF NOT EXISTS (SELECT 1 FROM public.horses LIMIT 1) THEN
...
DO $$
BEGIN
  IF NOT EXISTS (SELECT 1 FROM public.tracks LIMIT 1) THEN
